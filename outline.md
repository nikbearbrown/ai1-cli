...
> **AMENDMENT (2026-07-01, logged, not drift):** a new **Chapter 13 — From Book to Video** is added after shipping, per author direction: a finished, signed book is raw material for video, and the reader should meet **Unreal Reels** (AI+1's sibling video system) as an extension of the same discipline. Video is genuinely downstream of GATE-6/ship, so it sits after Part III, before the coda. Chapter count 12→13 (with the earlier Ch-2 research insertion, 13→14) — still inside Blueprint's 12–14 target. The chapter file exists: `chapters/13-from-book-to-video.md` (sidecar stubbed, awaiting sign-off).

13. **From Book to Video** — A finished, signed book is raw material for video. Meet **Unreal Reels**, AI+1's sibling system: the same two ideas (audio is the master clock; the human signs every gate), pointed at video instead of prose, over one file — `beat_sheet.json` — the way the book runs over `facts.json`. The reader turns one finished chapter into a narrated **lecture spine** (three stages: build a visual pool → select into a deck and sign the plan → narrate and preview), runs the **fidelity pass** (the video is downstream of a GATE-4 fact-checked chapter, so accuracy is a fidelity check, not a fact-check from scratch), and signs the lecture the way they sign a chapter — a video as a first-class AI+1 artifact. Referenced, not bundled, like Medhavy in Ch 12. Capability: extend a finished book into video without losing its authority — the same gated, signed, traceable discipline applied to a new medium.
...
> **AMENDMENT (2026-07-01, logged, not drift):** the 21 old-draft chapter bodies referenced by this mapping have been ported from `ai1-cowork`, reframed for the Cowork app (CLI/terminal → Cowork desktop; Tic-TOC → Blueprint), and **staged as source in `pantry/cowork-source/`** (with a `README.md` keying each file to its target above) — *not* left as standalone chapters, so `chapters/` keeps the restructured structure. They are seed material for the "why it works this way" overviews; Chapters 3–12 are still to be drafted (Phase 2) from this seed **plus** the copywriting running project — the old generic-process prose seeds overviews, but each chapter's exercises must be authored around writing-guide → copywriting, not lifted. `chapters/` currently holds Intro, Ch 1, Ch 2, Ch 13, and appendices 80–97.
