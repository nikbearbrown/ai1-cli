# Figure & Claim Accuracy Review — ai1-cli

*Companion to the layout linter. Layout = geometry (run `npm run audit:layout` first). This = substance.*

## Ground truth for this book

**Expert persona to adopt:** an ML/AI engineer and computer-science educator.

**Domain:** applied AI / CLI tooling and the computer-science concepts behind it.

**Authoritative sources to check claims against:**
- Correct CS/ML fundamentals: algorithmic complexity (Big-O), probability and linear-algebra identities, model behavior described accurately (no overclaiming what a model "knows" or guarantees).
- Library / API / CLI behavior must match the actual tool's documented behavior and current version.
- Reproducibility: any benchmark, runtime, or accuracy figure must state its setup and be internally consistent.

**Recompute, don't trust the picture:** complexity claims, math, and any quoted metric; verify pseudocode/code does what the text says. A wrong complexity class, broken identity, or incorrect API claim is `[CRITICAL]`.
