...
**When to use Claude in chat vs. a Claude Project vs. Cowork.** Three surfaces, each paired with a part of the freelance workflow: Claude in chat for brief reading and portfolio audits (roughly 80% of LLM use for this reader), a Claude Project for ongoing client work where the conversation needs to remember the brand guide and past decisions (roughly 15%), and Cowork for the book-building activity the reader is doing right now — the desktop app that opens the book folder, executes the pipeline, and produces the artifacts (roughly 5%, high-leverage). A generic Chapter 00 would list capabilities. The AI+1 Chapter 00 names the workflows and estimates the distribution, because the reader needs a decision rule, not a feature list.
...
Open your book folder in Cowork and tell Cowork to run the "With LLMs" Curriculum Enrichment Generator through phase one (state detection) and phase two (Chapter 00 generation). Have it pause when it presents the candidate running projects. Read all three to five candidates.
...
After Cowork has run the generator through phase three, pull three LLM Exercises from three different chapters into a single document. Strip the chapter context. Apply the three-question audit to each:
