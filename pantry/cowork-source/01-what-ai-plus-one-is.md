...
A boutique brand designer with eight years on one anchor account receives a request from a new VP of marketing. They want a brief for a new advisory product. The designer opens their project folder in Cowork and tells it what they want: *Draft a creative brief for a new healthcare advisory product. Modern, calm, credible, not hospital. Aimed at C-suite buyers. About 500 words.* Forty seconds later the brief is on screen. Polished headings. Project Overview. Target Audience. Brand Personality. Deliverables. Timeline. Success Metrics. The tone is professional. The language is industry-appropriate. The designer skims it, nods, and sends it to the VP.
...
The intellectual ancestor is Daugherty and Wilson's *Human + Machine* (2018), which named "the missing middle" — work that neither pure humans nor pure machines do well, but that hybrid teams handle.[^daugherty] Frey and Osborne's 2017 paper on the automation of jobs identified three durable bottlenecks where machines still fail: perception and manipulation in unstructured environments, creative intelligence, and social intelligence.[^frey] The AI+1 frame is what happens when you take those bottlenecks seriously and structure your practice around them rather than waiting to see how they resolve.

The empirical case that AI+1 is economically real — not just a comfortable story — sits in the labor market data. PwC's 2025 *AI Jobs Barometer* reports that revenue per employee in AI-exposed industries grew nearly four times faster than in less-exposed sectors, and that workers with advanced AI skills command a wage premium of roughly 56% in PwC's US data.[^pwc] Lightcast's 2025 job-posting analysis found that postings mentioning AI skills offered approximately 28% higher salaries — roughly $18,000 more per year — than comparable postings without.[^lightcast] These numbers come with caveats. The PwC figure is contested: critics argue the premium reflects survivor selection bias rather than causal benefit, because workers who keep their jobs in AI-exposed industries are systematically the most skilled to begin with.[^pwc-critique] The 28% Lightcast figure varies by sector and is geographically uneven [contested — see pantry flag]. The figure below therefore treats the bars as related evidence, not interchangeable measurements: PwC is a worker-skill cut, Lightcast is a job-posting salary cut, and the lower contested cut marks sector/geography sensitivity rather than a third universal estimate.
...
Michael Polanyi wrote in 1966 that "we can know more than we can tell."[^polanyi]
<!-- FACT-CHECK FLAG: UNVERIFIED — see factchecks/01-what-ai-plus-one-is-assertions.md -->
 He was describing tacit knowledge — the things professionals do well that cannot be reduced to a written rule. Donald Schon picked up the same thread in 1983 and named it *reflection-in-action*: the way a professional thinks *while doing*, adjusting as the situation talks back.[^schon]
<!-- FACT-CHECK FLAG: UNVERIFIED — see factchecks/01-what-ai-plus-one-is-assertions.md -->
...
<!-- → [TABLE: irreducibly human taxonomy — two columns: competency and why AI cannot replace it — eight rows covering client intuition, taste as subtractive judgment, creative accountability, brief interpretation, constraint navigation, presentation and persuasion, cultural reading, brand stewardship] -->
...
The three-LLM domain research prompt is the move that catches what one model misses. You run the same structured prompt across Claude, GPT, and Gemini. Where all three agree, you have settled territory — but only provisionally, because the models are not cleanly independent witnesses. Where they diverge, you have contested ground worth flagging. Where one raises a point the others missed, you have a candidate for your synthesis. This is investigator triangulation in Norman Denzin's 1978 vocabulary — using multiple independent sources to catch what any single one misses.[^denzin]
<!-- FACT-CHECK FLAG: UNVERIFIED — see factchecks/01-what-ai-plus-one-is-assertions.md -->
 The three frontier LLMs satisfy the independence condition weakly (they overlap enormously in training data) but better than any single model does on its own.
...
Paste this verbatim to Cowork with web research enabled. Run it verbatim in ChatGPT. Run it verbatim in Gemini. Save all three outputs in plain markdown. You will get three documents that are recognizably about the same field and confidently disagree on several specifics. That disagreement is the value. Your job is to read across the three and produce a synthesis — which is what Chapter 3 walks through in detail.
...
Where Gemini was most specific and the other two were not: adoption statistics. Ninety-three percent of graphic designers use AI-powered tools at least once a week; 82% use them to overcome blank-canvas syndrome; only 12% trust these tools to handle high-stakes branding for Fortune 500 companies [verify — Figma State of the Designer 2026]. **Reading move:** the most specific number is not necessarily the most accurate. Verify the source before quoting in your book. Treat Gemini's numbers as candidates for verification, not as facts.

Where Gemini surfaced something the other two missed: the IP doctrine. *Nemo dat quod non habet* — you cannot give what you do not have. A designer who generates a logo entirely with AI possesses no copyright in that work. Any standard IP assignment in the designer's contract transfers nothing to the client. Neither Claude nor GPT named the legal doctrine; Gemini retrieved it. This is divergent-as-gap — a real piece of information one model retrieved that the others missed. **Reading move:** verify the legal claim against the Copyright Office reports and the current case law [verify — Thaler v. Perlmutter, D.C. Circuit, 2025], then include in the brief with attribution.
...
The three-LLM research prompt is how you discover where your own boundary currently sits. Not my boundary — yours. For your field. With your clients. Right now, in the current state of the tools. The chapter ends here because the next thing you need is not more analysis. It is the output of your own three-LLM run, sitting in three markdown files, ready for Chapter 3.
...
Use the prompt template from this chapter. Adapt the bracketed field to your profession. Run it verbatim in Cowork (with web research enabled), GPT-4 or later, and Gemini. Save all three outputs as plain markdown.

**Time required:** 45–60 minutes including running the prompts, copying outputs, and saving files.

**Deliverable:** Three files: `claude-output.md`, `gpt-output.md`, `gemini-output.md`. No synthesis yet — that is Chapter 3.

### Exercise 2 (Analyze) — Identify three fluency-trap examples from your research output

Read the three outputs from Exercise 1. Find three examples of AI output failure modes specific to your field. For each, write: the failure pattern (one sentence); why a non-expert would not notice it (one sentence — this is what makes it a fluency trap rather than a regular bug); what the expert sees that the non-expert misses (one sentence — this is the irreducibly human layer).

**Deliverable:** A 200-word memo, three examples, roughly 60 words each.

### Exercise 3 (Evaluate) — Assess whether AI is creating a wage premium or displacement risk in your field

Re-read section 5 of each of the three LLM outputs. Identify the strongest piece of evidence for a wage premium for AI-fluent practitioners in your field, and the strongest piece of evidence for displacement. Write 100–200 words giving your considered judgment about which signal is currently stronger in your specific subfield, and why. There is no correct answer — there is a defensible answer, supported by your reading of the research output and your domain expertise.
...
>
> "Read the Wikipedia article on Lucy Suchman. In 300 words, explain how her concept of 'situated action' applies to the way a freelance graphic designer makes judgment calls during a client call — judgments that an AI assistant cannot make for them."

Suchman is an anthropologist of work who argued, against the Stanford AI mainstream of the 1980s, that intelligent machines do not replace human judgment but *reconfigure* the work it does. Her 1987 book *Plans and Situated Actions* is the intellectual ancestor of the AI+1 frame. The prompt is short. The Wikipedia article is accessible. The deliverable is a one-page memo to keep alongside your delegation map.[^suchman]
...
The fluency trap is now felt and named. You can see it in your own field, in your own outputs, in your own drafts. You can name the irreducibly human layer that protects against it. You have the three-LLM research prompt and a sense of what the synthesized brief looks like when it is done.

That is the diagnosis. What you do not yet have is the structural response.

Blueprint is the structural response. The two-hour conversation that produces a BLUEPRINT.md is the architecture that catches the fluency trap before it becomes a chapter, then a book. Chapter 2 shows you the product — the ai-for-designers BLUEPRINT.md, in full — before it explains how the product was made.
...
### Figure 1.1 — Generic brief vs. client-informed brief
Build a two-panel comparison diagram as one standalone HTML file with inline CSS and D3 7.9.0 from the CDN. Data shape: two parallel documents, each with six stacked sections; the right document additionally carries six "knowledge gap" markers, one per section. Marks: filled rectangles (section bars) for both panels; on the right panel use faint filled section bars to show that surface content exists, then add six hollow ring markers in the margin connected by thin leader lines to the sections where institutional knowledge is missing. Channels: y position = section order (shared across panels); filled versus hollow marker = surface content versus missing institutional knowledge; left/right column = generic versus client-informed. No quantitative axis. Sort: sections top-to-bottom in document order. Annotate each gap marker with its short label (gradient grudge, PMS green, new VP, no budget, rival shops, six-person committee). Exactly six gaps. Deliverable: one self-contained HTML file, inline CSS, D3 7.9.0 CDN, responsive via ResizeObserver, accessible SVG with title/desc.

### Figure 1.2 — The fluency trap as a two-component mechanism
Build a single horizontal causal-chain diagram (systems diagram, not a chart) as one standalone HTML file with inline CSS and D3 7.9.0 from the CDN. Data shape: five fixed nodes plus one interruption marker — (1) model output: statistical coherence, no communicative intent; (2) reader cognition: processing-fluency reward; (3) bypassed "is it informed?" check; (4) closed-trap terminal; plus a judgment-interruption marker before trap closure. Marks: rounded rectangles for nodes, straight arrows for the main left-to-right chain, a curved feedback arc between nodes 1 and 2 (mutual amplification), a dashed connector to node 3 placed off the main line to encode "skipped," and a blockage glyph (⊣) on the final segment before the trap closes. Channels: x position encodes causal order; the dashed/off-line treatment encodes bypass; color marks only the primary chain (red) versus neutral supporting nodes. No quantitative axis, no zero baseline. Label every node and the interruption point as the escape route. Deliverable: one self-contained HTML file, inline CSS, D3 7.9.0 CDN, responsive via ResizeObserver, accessible SVG with title/desc.

### Figure 1.3 — Wage premium by sector and skill level
Build a vertical bar chart as one standalone HTML file with inline CSS and D3 7.9.0 from the CDN. Data shape: one series, three bars — PwC US advanced-AI-skills premium 56%, Lightcast AI-posting premium 28%, and a lower contested cut (~14%). Single value channel: percent wage premium, range 0–60. Marks: rectangles. Channels: x = source/cut (band scale), y = percent (linear, zero baseline required). Color: the two confirmed bars in the primary series color; the contested bar in a distinct non-red fill to flag it as uneven. Sort: descending by magnitude. Annotate each bar with its value label and source, and add a short methodological caveat that PwC worker-skill premiums and Lightcast job-posting salary premiums are related but not identical measures. No dollar figures mixed onto the percent axis. Deliverable: one self-contained HTML file, inline CSS, D3 7.9.0 CDN, responsive via ResizeObserver, accessible SVG with title/desc.

### Figure 1.4 — Three-LLM triangulation to synthesis
Build a top-to-bottom convergence diagram as one standalone HTML file with inline CSS and D3 7.9.0 from the CDN. Data shape: three source nodes (Claude, GPT, Gemini) feeding four claim-category regions feeding one synthesis node. Marks: three circles arranged as a three-way Venn at the top, arrows flowing downward into a single synthesis rectangle at the bottom, plus a small caveat band attached to the overlap region noting shared training data / weak independence. Channels: the overlap geometry encodes the four categories — central triple-overlap = all-three-agree, pairwise lenses = two-agree, single-circle crescents = one-only, and a forked marker beside the cluster = divergent. Color: one distinct hue per source circle, synthesis node in the primary series color; overlap outlines neutral. No quantitative axis, no zero baseline. Label the three sources, the four category regions, the weak-independence caveat, and the synthesis node. Deliverable: one self-contained HTML file, inline CSS, D3 7.9.0 CDN, responsive via ResizeObserver, accessible SVG with title/desc.

---

## References

The following sources were verified during fact-checking and support confirmed factual claims in this chapter. See `factchecks/01-what-ai-plus-one-is-assertions.md` for the full report.
...
3. Frey, C. B., & Osborne, M. A. (2017). "The Future of Employment: How Susceptible Are Jobs to Computerisation?" *Technological Forecasting and Social Change*, 114, 254–280. https://www.sciencedirect.com/science/article/pii/S0040162516302244
4. Daugherty, P. R., & Wilson, H. J. (2018). *Human + Machine: Reimagining Work in the Age of AI.* Harvard Business Review Press. https://www.accenture.com/us-en/insights/technology/human-plus-machine
5. PwC. (2025). *The Fearless Future: 2025 Global AI Jobs Barometer.* PwC. https://www.pwc.com/gx/en/services/ai/ai-jobs-barometer.html
6. Lightcast. (2025). *Beyond the Buzz: Developing the AI Skills Employers Actually Need.* Lightcast. https://lightcast.io/resources/blog/beyond-the-buzz-press-release-2025-07-23
7. Bloomberry. (2025). "I analyzed 180M jobs to see what jobs AI is actually replacing today." https://bloomberry.com/blog/i-analyzed-180m-jobs-to-see-what-jobs-ai-is-actually-replacing-today/
8. Brynjolfsson, E. (2022). "The Turing Trap: The Promise & Peril of Human-Like Artificial Intelligence." *Daedalus*, 151(2), 272–287. https://direct.mit.edu/daed/article/151/2/272/110622/The-Turing-Trap-The-Promise-amp-Peril-of-Human
9. Suchman, L. A. (1987). *Plans and Situated Actions: The Problem of Human-Machine Communication.* Cambridge University Press. https://www.cambridge.org/core/books/plans-and-situated-actions/
