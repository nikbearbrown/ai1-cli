...
Before the three operations, a note on tooling. CAJAL's SVG generator and the PNG converter at `SCRIPTS/svg-to-png.mjs` are Node.js programs — and the good news is that you never touch them directly. Cowork runs them for you. You do not open a terminal, you do not learn Node, and you do not read any JavaScript. You open your book folder in Cowork, tell Cowork you want to run the figure pipeline, and Cowork executes the scripts on your machine and reports back.

Cowork does need Node 18 or higher present on your machine so the scripts can run. If it is not already there, Cowork can install it for you — tell Cowork to set up Node, and it will run the right step for your system:

**Mac:** Cowork runs `brew install node` and then verifies with `node --version`.

**Windows:** Cowork runs `winget install OpenJS.NodeJS.LTS` and then verifies with `node --version`.

**Linux (Ubuntu/Debian):** Cowork runs `sudo apt update && sudo apt install -y nodejs npm` and then verifies both.

Then, from inside your book folder, Cowork runs `cd SCRIPTS/ && npm install sharp` to install the PNG converter's one dependency. If it fails on Mac silicon with a message about Python or libvips, Cowork reruns with `npm install --include=optional sharp`. That is the only workaround you will need. [verify — `sharp` install paths can drift between major versions on macOS]

You do not need to learn Node. You do not need to read any JavaScript. You need it on your machine so the scripts Cowork runs will work. Five minutes, and Cowork does the typing.
...
The arrow is a grep target so Cowork can find all pending visuals at once. The bracket type — `INFOGRAPHIC`, `CHART`, `TABLE`, `IMAGE`, `DIAGRAM` — tells the enrichment pass which generator to invoke. The description after the colon is the brief the figure-generation step reads.
...
|---|---|---|
| The arrow | A grep target so every pending visual can be found at once | `→` |
...
**Exercise 9.1 (Apply) — Run the Chapter Finishing Pass.** Open your book folder in Cowork and tell Cowork to run the Chapter Finishing Pass. Confirm every chapter has a one-line italic subtitle and at least two visual placeholder comments. For each subtitle, write one sentence answering: *what argument does this subtitle commit the chapter to?* If you cannot answer in one sentence, the subtitle is a territory label. Rewrite it.
...
**Exercise 9.2 (Apply) — Run CAJAL Image Suggest; audit one `cajal.md`.** Tell Cowork to run CAJAL Image Suggest. Open the `cajal.md` for one chapter — pick the chapter you are most uncertain about visually. Confirm the Critical-ranked figure maps directly to one of the chapter's primary learning outcomes from your Blueprint.md. If it does not, rewrite the SCOPE prompt until it does, or demote it. Identify one Supplementary-ranked figure you will skip and note the reason.
